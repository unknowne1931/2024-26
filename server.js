...
let transporter2 = nodemailer.createTransport({
  ...
  auth: {
    ...
    pass: 'lkdq afqf rsby laww' // Replace with your Gmail password or app-specific password
  }
});
...
const sendEmail = (error) =>{
  // Email content
  let mailOptions = {
      from: 'krishnakick1931@gmail.com',
      to: 'darshanckick@gmail.com', // Replace with recipient's email address
      subject: 'Error Found',
      text: `Error: ${error}`, // Plain text body
      html: `
      <html lang="en">
      <head>
          <meta charset="UTF-8">
      ...
      <body>
          <div class="container">
              <h2>Error Found</h2>
              <p>There was an error: <strong>${error}</strong></p>
              <p>Please fix it as soon as possible.</p>
          </div>
      </body>
      </html>
      ` // HTML body
                ...
                <div class="email-main-cnt-01">
                    ...
                    <div class="email-cnt-02">
                        ...
                        <p>Your Account need Attention to Change<br />
                            ...
                            <a href = "https://durgansathleticsacademy.in/password?token=${token}&username=${User.Username}&page=true" style="text-decoration: none;">Chnage Password</a>
                        <strong></strong><br/>
...
app.get("/app/get/data/by/id/:id", async (req, res) => {
  const id = req.params.id;
  try {
    const User = await PhotoModule.findById({_id : id}); // Use 'id' directly instead of {_id: id}
    if (User) {
      return res.json({ User });
    } else {
...
const ModuleGF = mongoose.model('google_form', evebtSchema);

const storage7 = multer.diskStorage({
  destination : function (req, file, cb) {
    cb(null, "public/google");
  },
  filename : function (req, file, cb) {
    const uniqueSuffix = Date.now();
    cb(null, uniqueSuffix+ file.originalname);
...
app.post("/googleform/get/post/data", upload7.single("image") ,async (req, res)=>{
  const image = req.file ? req.file.filename : null;
  const {gf} = req.body;
  try{
    await ModuleGF.create({image : image, Time, gf})
    return res.json({Status : "OK"})
  } catch (error){
    sendEmail(error)
    console.log(error)
...
app.get("/get/data/post/events/ss", async (req, res) => {
  try {
    const data = await ModuleGF.find({});
...
app.post("/delete/event/poster/googlel/data", async (req, res) =>{
  const {id} = req.body;
  try{
    await ModuleGF.findByIdAndDelete({_id : id})
    return res.json({Status : "OK"})
  }catch (error){
    sendEmail(error)
    console.log(error)
